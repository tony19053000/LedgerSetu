...
export default function SelfHostedSetupFormClient({ defaultProvider, defaultApiKeys }: Props) {
  ...
  return (
    <form action={selfHostedGetStartedAction} className="flex flex-col gap-8 pt-8">
      ...
      <div>
        <small className="text-sm text-muted-foreground flex justify-center mt-2 text-center">
          AI Provider configured securely by the server via environment variables.
        </small>
        <input type="hidden" name={selected.apiKeyName} value="" />
      </div>
